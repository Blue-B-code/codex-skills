...
- `/home/y-note/openimis*` → skill **openimis**
- `/home/y-note/OrangeMoney/MobileInvoice` → skill **mi**
- `/home/y-note/flownote` → skill **flownote**
...
## Validation
- La gate de validation est celle du **skill projet** (`mi` / `openimis` / `flownote`) : lire ce skill et appliquer **sa** validation (conteneur MI, uv/pytest flownote, phing/vitest openimis…).
- Appliquer de façon **ciblée** (fichiers touchés) et **fail-fast** (`timeout` via config `validation.timeout_seconds`, défaut 900 s) : si l'environnement est cassé (conteneur instable, donnée DB absente), **rapporter la cause et s'arrêter** — pas de retry, pas de hang, pas de push en masse.
